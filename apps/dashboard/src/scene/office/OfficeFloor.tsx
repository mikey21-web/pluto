...
export function OfficeFloor() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const theme = await loadTheme(officeTheme as import('./themeRegistry').ThemeId);
      await app.init({
      ...
      const updateBossAura = (dt: number): void => {
        ...
        if (statsAge > 30) {
          statsAge = 0;
          // ponytail: stub — no task ledger in Pluto web build
          void Promise.resolve(null).then(() => { /* keep last counts */ });
        }
        auraCooldown -= dt;
      ...
      const pollTaskBoard = async (): Promise<void> => {
        try {
          // ponytail: stub — no task ledger in Pluto web build
          const arr: Array<{ id?: string; status?: string; assignee?: string; humanQA?: Array<{ q?: string; a?: string }> }> = [];
      ...
      // ponytail: stub — no hive IPC in Pluto web build
      const offMessage = () => { /* onHiveMessage unavailable — Pluto web build */ };
